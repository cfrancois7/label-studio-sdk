...
def convert_to_absolute(
    xyxyxyxy: list[float], image_width: float | int, image_height: float | int
) -> list[float]:
    absolute_coords = []
    for i in range(len(xyxyxyxy)):
        if i % 2 == 0:  # x-coordinate
            absolute_coords.append(xyxyxyxy[i] * image_width)
        else:  # y-coordinate
            absolute_coords.append(xyxyxyxy[i] * image_height)
    return absolute_coords


def convert_yolo_obb_to_ls(
    input_dir,
    out_file,
    to_name="image",
    from_name="label",
    ...
    """Convert YOLO OBB labeling to Label Studio JSON
        ...
        if os.path.exists(label_file):
            ...
            with open(label_file) as file:
                ...
                for line in lines:
                    values = line.split()
                    label_id = values[0]
                    xyxyxyxy = convert_to_absolute(
                        [float(v) for v in values[1:9]], image_width, image_height
                    )
                    score = float(values[9]) if len(values) >= 10 else None
                    ...
                    item = {
                        ...
                        "value": {
                            "x": ls_yolo_obb_dict["x"],
                            "y": ls_yolo_obb_dict["y"],
                            "width": ls_yolo_obb_dict["width"],
                            "height": ls_yolo_obb_dict["height"],
                            "rotation": ls_yolo_obb_dict["rotation"],
...
def add_parser(subparsers):
    yolo = subparsers.add_parser("yolo_obb")
    ...
    yolo.add_argument(
        "--image-ext",
        dest="image_ext",
        help="image extension to search: .jpeg or .jpg, .png, .bmp",
        default=".jpg",
    )
    yolo.add_argument(
        "--image-dims",
        ...
        help=(
            ...
            "case where you dataset has uniform image dimensions. e.g. `--image-dims 600 800` "
